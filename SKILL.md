...
When an exact project is found, tell the user its name and ID and present explicit choices for the repository-root `.openproject.json` decision; do not ask an open-ended question. Offer: **Yes — create/update with this project ID**, **No — use it only for this request**, and **Use a different project ID — let the user type an ID**. Do not create or modify the file without the user's permission. If the current request already explicitly authorizes this configuration change, do not ask again. Once authorized, create the file when absent or update it when present, preserving `host` and other supported settings, then use the persisted `project_id` for subsequent relevant queries. Treat this local configuration change separately from OpenProject API writes.
